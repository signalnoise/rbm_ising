...
def ising_averages(mag_history, model_size, label=""):
    ...
    """
    mag_err = model_size*((mag_history[:,0]-mag_avg[0,0])**2)
    susc_avg = mag_err.mean(axis=0, keepdims=True)
    susc_err = mag_err.std(axis=0, keepdims=True)/sqrt(sample_size)
    """
    print(mag_history.shape)
    susc = np.var(mag_history[:,0]*model_size)/(model_size*2.27)
    print(label, " ::: Magnetization: ", mag_avg[0,0], " +- ", mag_std[0,0]/sqrt(sample_size), " - Susceptibility:", susc)
...
if parameters['initialize_with_training']:
    ...
    for i in range(parameters['concurrent samples']):
        data[i] = train_loader[randint(0,len(train_loader[:][0]))][0].view(-1, model_size)
